export STARPU_PATH=/home/alecrim/starpu
export STARPU_CONFIG_FOLDER="config_cuda" 
export STARPU_PATH_AUDIO_FFT_EXAMPLES=/home/alecrim/devel/StarPU/examples/audio
export PKG_CONFIG_PATH=$STARPU_PATH/lib/pkgconfig
export LD_LIBRARY_PATH=$STARPU_PATH/lib
export STARPU_EXAMPLES_DIR=$STARPU_PATH/lib/starpu/examples
export STARPU_NCPU=1
export STARPU_NCUDA=1
export STARPU_OPENCL=0

